...
alg = GaussianNB() #.fit(cc[predictors],cc['tag'])
...
File = open("data/prob-NB.txt", "w",encoding=u'utf-8', errors='ignore')
File.write("id"+",")
File.write("prob" + "\n")
classifier = alg.fit(cc[predictors], cc['tag'])
predictiontest = classifier.predict_proba(test)[:,1]
...
#scores.mean= 0.8682307536091821  n_estimators=2000  n_splits=60