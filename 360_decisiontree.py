...
Xtest = pd.read_csv("data/decisiontree.txt", sep='\t')
x_data_output = dfp.iloc[:, 0:1].values
#print(data_train.iloc[:, 3:4])
#
#

predictors = pd.read_csv("data/decisiontree.txt", sep='\t')
# 62棵决策树，停止的条件：样本个数为2，叶子节点个数为1
alg = DecisionTreeClassifier(criterion='gini', splitter='best', max_depth=None, min_samples_split=2,min_samples_leaf =1,
...
File = open("data/prob_decisiontree.txt", "w",encoding=u'utf-8', errors='ignore')
File.write("id"+",")
File.write("prob" + "\n")
